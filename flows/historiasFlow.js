...
const createHistoriasFlow = (provider, { unknownFlow } = {}) => {
  ...
  const historiasInteresadoSi = addKeyword(EVENTS.ACTION)
    .addAnswer(
      enviarFormularioHistoriasRandom(),
      { delay: 5000 },
      async (ctx, { endFlow }) => {
  ...
  const pedirRespuestaCorrectaHistorias = addKeyword(EVENTS.ACTION)
    .addAnswer(
  ...
  const historiasFlow = addKeyword(['historia', 'historias', 'instagram'])
      ...
      async (ctx, { flowDynamic, gotoFlow, endFlow }) => {
        try {
          ...
          if (userResponse.includes('1') || userResponse.includes('si')) {
            ...
            // Respuesta no reconocida - Redirigir al flujo de respuestas desconocidas
            console.log('Respuesta no reconocida, redirigiendo al flujo de respuestas desconocidas');
            return gotoFlow(pedirRespuestaCorrectaHistorias);
          }
        } catch (error) {
...
module.exports = createHistoriasFlow;